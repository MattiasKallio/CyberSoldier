	...
	document.addEventListener('deviceready', function() {
		...
		if (lt != "Logout") {
			...
			function(response) {
			    ...
			    // Just to set anything because iPhone es mundu
			    // dumbo.
...
function setCharacterBaseItems() {
	...
	function txede_first(tx) {
		...
		tx.executeSql('INSERT INTO character_items (id, type, name, icon, svg_info, date_added) VALUES (' + clizt[i].id + ',"' + clizt[i].type + '","'
		        + clizt[i].name + '","' + clizt[i].icon + '","' + svg_cl + '","' + clizt[i].date_added + '")');
	    }
...
function doLogout() {
    ...
    /*
     * facebookConnectPlugin.logout( function(response){ alert("logout:
     * "+JSON.stringify(response)); }, function(response){ alert("logout fejl :
     * "+JSON.stringify(response)); } );
     */
